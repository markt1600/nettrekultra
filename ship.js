...
// short-radar range of home. Sets friendlies to RESCUE mode. Pressing Q
// (or clicking the SOS button) while a rescue is already inbound CANCELS
// the call instead, returning friendlies to normal duty.
const SOS_RESCUE_DURATION = 10.0;   // seconds friendlies hang around when no enemies

// True if any friendly ship is currently in RESCUE mode targeting the player.
function isSOSActive(me, world) {
  if (!me) return false;
  for (const f of world.ships) {
    if (f.team !== me.team || !f.alive || f.isPlayer) continue;
    if (f.aiState && f.aiState.mode === "RESCUE" && f.aiState.rescueTarget === me.id) return true;
  }
  return false;
}

// Original "wave-off" lines so the cancel doesn't feel canned.
const SOS_CANCEL_LINES = [
  "Crisis averted — stand down, friends.",
  "False alarm — back to your patrol routes.",
  "Threat neutralized — cancel the SOS.",
  "Situation under control — disregard the call.",
  "All clear — appreciate the run.",
  "Cancel SOS — we're holding the line.",
];

function cancelSOS(me, world) {
  if (!me) return 0;
  let count = 0;
  for (const f of world.ships) {
    if (f.team !== me.team || !f.alive || f.isPlayer) continue;
    if (!f.aiState || f.aiState.mode !== "RESCUE" || f.aiState.rescueTarget !== me.id) continue;
    f.aiState.mode = "HUNT";  // aiChooseMode will pick something appropriate next think
    f.aiState.rescueTarget = null;
    f.aiState.rescueUntil = 0;
    count++;
  }
  const line = SOS_CANCEL_LINES[Math.floor(Math.random() * SOS_CANCEL_LINES.length)];
  pushMessage(world, `${line} (${count} friendly ship${count === 1 ? "" : "s"} resuming patrol).`, "you");
  return count;
}

// Original "no help" wave-off lines when nobody can answer the SOS.
const SOS_NO_HELP_LINES = [
  "No help available — you're on your own.",
  "Comms silent — no one's coming. Survive.",
  "Friendly fleet unavailable — make your stand.",
  "All friendlies engaged elsewhere — fight on alone.",
];

function triggerSOS(me, world) {
  if (!me || !me.alive) return false;
  // Already calling for help → second press cancels.
  if (isSOSActive(me, world)) { cancelSOS(me, world); return true; }
  const home = world.planets.find(p => p.origTeam === world.playerTeam && (p.flags & FLAG_HOME));
  if (!home) {
    if (typeof flashSOSAlert === "function") flashSOSAlert("NO HELP AVAILABLE", "No home base — you're on your own.");
    pushMessage(world, "SOS failed — no home base.", "alert");
    return false;
  }
  const dHome = Math.hypot(home.x - me.x, home.y - me.y);
  if (dHome > RADAR_SHORT_RANGE) {
    if (typeof flashSOSAlert === "function") {
      flashSOSAlert("SOS OUT OF RANGE",
        `You are ${Math.round(dHome)}u from home. Get within ${RADAR_SHORT_RANGE}u to signal.`);
    }
  ...
  if (count === 0) {
    const line = SOS_NO_HELP_LINES[Math.floor(Math.random() * SOS_NO_HELP_LINES.length)];
    if (typeof flashSOSAlert === "function") flashSOSAlert("NO HELP AVAILABLE", line);
    pushMessage(world, `SOS sent — ${line}`, "alert");
  } else {
    pushMessage(world, `SOS sent — ${count} friendly ship(s) en route. Press Q again to cancel.`, "you");
  }
  return true;
}
...
function autoFireAtTarget(s, world) {
  ...
  if (inPhaserCone(s, t) && d <= def.phaserRange &&
      ...
      (world.now - (s._autoPhaserLastAt || 0)) >= phaserAutoInterval(s)) {
    if (firePhaser(s, world, t)) s._autoPhaserLastAt = world.now;
  }
  // Torpedoes: narrower cone (±20°). Auto-fire enforces TWO independent caps:
  //   1) Rate: torpAutoBurstSize per torpAutoBurstWindow seconds (DG = 5/sec)
  //   2) Per-lock total: torpAutoMaxPerLock torps against any single target
  //      before requiring a fresh lock (DG = 10, others = 3). The counter
  //      resets when targetLock changes — clicking the same enemy after the
  //      lock dropped counts as a fresh lock.
  const burstSize = def.torpAutoBurstSize || AUTO_TORP_BURST_SIZE;
  const burstWindow = def.torpAutoBurstWindow || AUTO_TORP_BURST_WINDOW;
  const maxPerLock = def.torpAutoMaxPerLock || 3;
  if (s._torpsAgainstTargetId !== s.targetLock) {
    s._torpsAgainstTargetId = s.targetLock;
    s._torpsAgainstCount = 0;
  }
  if (inFiringCone(s, t) && d <= def.torpRange &&
      s.torpCool === 0 && s.torpCount > 0 && s.energy >= def.torpEnergy &&
      (s._torpsAgainstCount || 0) < maxPerLock) {
    if (!s._autoTorpBurstStart || (world.now - s._autoTorpBurstStart) >= burstWindow) {
      s._autoTorpBurstStart = world.now;
      s._autoTorpBurstCount = 0;
    }
    if ((s._autoTorpBurstCount || 0) < burstSize) {
      const aimAng = Math.atan2(t.y - s.y, t.x - s.x);
      if (fireTorp(s, world, aimAng, t)) {
        s._autoTorpBurstCount = (s._autoTorpBurstCount || 0) + 1;
        s._torpsAgainstCount = (s._torpsAgainstCount || 0) + 1;
      }
    }
  }
}
...
function deployFlare(s, world, maxPerTorp) {
  ...
  if (s.isPlayer && typeof flashWeaponBtn === "function") flashWeaponBtn("btn-fire-flare");
