...
function aiEngage(s, world, target) {
  ...
  // seconds and matches the player's Auto-Fire rate. Cadet bumps the AI
  // interval to 0.75s so a new player has more breathing room.
  const phInt = world.aiCadetSlowdown ? 0.75 : phaserAutoInterval(s);
  if (d < def.phaserRange && s.phaserCool === 0 &&
      s.energy > phaserEnergyCost(s, world) * 1.5 &&
      (world.now - (s._autoPhaserLastAt || 0)) >= phInt) {
  ...
  // Cadet adds a 1-second minimum between torps from any one enemy.
  // Per-target cap (torpAutoMaxPerLock = 3 for SC/DD/CA/BB): once we've put
  // 3 torps at a particular target ship, we don't throw more until the AI
  // shifts to a different engagement target (counter resets on target id
  // change). Demi God's 10 doesn't apply here — AI never pilots a DG.
  const maxPerLockAi = def.torpAutoMaxPerLock || 3;
  if (s._torpsAgainstTargetId !== target.id) {
    s._torpsAgainstTargetId = target.id;
    s._torpsAgainstCount = 0;
  }
  const tpGate = !world.aiCadetSlowdown
    || (world.now - (s._autoTorpLastAt || 0)) >= 1.0;
  if (d < RADAR_SHORT_RANGE && s.torpCool === 0 && s.torpCount > 0 &&
      s.energy > def.torpEnergy * 1.5 && facingErr < 0.45 && tpGate &&
      (s._torpsAgainstCount || 0) < maxPerLockAi) {
    if (fireTorp(s, world, aimAng, target)) {
      s._autoTorpLastAt = world.now;
