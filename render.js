...
function drawShip(ctx, s, sx, sy, scale, world) {
  ...
  // Scale the ship size with the tactical zoom so a zoomed-out view doesn't
  // show giant ships filling the screen. def.radius is calibrated for zoom 1×;
  // we scale linearly with the px/u ratio (so 8× zoom shows ships 1/8 the
  // pixel size) with a small floor so ships never become unreadable specks.
  const baseScale = ctx.canvas.width / TACTICAL_RANGE;
  const zoomFactor = scale / baseScale;
  const r = Math.max(5, def.radius * zoomFactor);
  ...
  if (isMe) {
    ...
    ctx.font = "10px Courier New";
    ctx.textAlign = "center";
    ctx.fillText(`${s.team}-${s.shipClass}`, sx, sy - r - 8);
    ctx.textAlign = "left";
  }
