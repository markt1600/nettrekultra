...
function loop(ts) {
  try {
    ...
    // Rate-limit the in-game chatter so a persistent bug doesn't spam every
    // frame. Browser console always logs (open DevTools → Console for the
    // stack trace).
    console.error("Game loop error:", e);
    if (world && (world.now - (world._lastErrChatterAt || -10)) > 5) {
      world._lastErrChatterAt = world.now;
      const detail = (e && e.message) ? `: ${String(e.message).slice(0, 60)}` : "";
      pushMessage(world, `Bridge fault${detail} — open browser console for details. (Gameplay continues.)`, "alert");
    }
  }
...
function startGame() {
  ...
  // Cadet: throttle enemy weapon cadence so the new player isn't overwhelmed.
  // Phaser auto-interval bumped from 0.5s → 0.75s; torpedoes capped at 1/sec.
  world.aiCadetSlowdown = (diffRaw === "cadet");
  attachInput(world);
  ...
  // and core loop at a glance. Pauses the game until dismissed. The "engage"
  // banner that follows lands AFTER the briefing closes for Cadet, immediately
  // otherwise.
  if (diffRaw === "cadet") openCadetBriefing();
  else flashEngagePrompt();
}
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // Radar mode segmented toggle (SHORT / LONG). Clicking a segment sets that
  // mode; clicking the border around them just toggles.
  const rt = document.getElementById("radar-toggle");
  if (rt) rt.addEventListener("click", (e) => {
    if (!world) return;
    const opt = e.target.closest(".seg-opt");
    if (opt) {
      world.radarMode = opt.getAttribute("data-mode");
    } else {
  ...
  btn("btn-autolocknav", () => {
    ...
    if (world.autoLockAndNavigate) dismissEngagePrompt();
...
function closeCadetBriefing() {
  ...
  el.classList.add("hidden");
  if (world) world.paused = false;
  // Now that the briefing is gone, surface the call-to-action prompt.
  flashEngagePrompt();
}
