...
function updateHud(world) {
  ...
  // ---- Speed meter on left of tactical (vertical fill 0–9) ----
  const speedFill = document.getElementById("speed-fill");
  if (speedFill) {
    const pct = Math.min(100, (me.speed / 9) * 100);
    speedFill.style.height = `${pct}%`;
  }
  setText("speed-val", Math.round(me.speed));

  // ---- Shields button reflects current state (UP / DOWN). ----
  const btnShields = document.getElementById("btn-shields");
  if (btnShields) {
    btnShields.classList.toggle("shields-up", !!me.shieldsUp);
    btnShields.classList.toggle("shields-down", !me.shieldsUp);
  }
  setText("btn-shields-state", me.shieldsUp ? "UP" : "DOWN");
...
// Brief visual flash on a weapon button when a phaser or torpedo fires.
// Called from the fire functions in weapons.js (player-only).
function flashWeaponBtn(id) {
  const el = document.getElementById(id);
  if (!el) return;
  el.classList.add("firing");
  if (el._flashTimeout) clearTimeout(el._flashTimeout);
  el._flashTimeout = setTimeout(() => el.classList.remove("firing"), 180);
}

// Sync the SHORT / LONG segmented toggle in the radar header to world.radarMode.
function syncRadarToggle(world) {
  const opts = document.querySelectorAll("#radar-toggle .seg-opt");
  opts.forEach(o => o.classList.toggle("active", o.getAttribute("data-mode") === world.radarMode));
}

// Flash a red SOS-failure banner at the top of the tactical view.
// title is the headline (uppercase); sub is an optional second line of detail.
function flashSOSAlert(title, sub) {
  const el = document.getElementById("sos-alert");
  if (!el) return;
  el.innerHTML = sub
    ? `${title}<span class="sos-alert-sub">${sub}</span>`
    : title;
...
// Yellow welcome banner shown briefly at game start telling the user how
// to begin fighting. Auto-hides after 6 seconds or when dismissed manually
// (via dismissEngagePrompt — called from AL+N toggle).
function flashEngagePrompt() {
  const el = document.getElementById("engage-prompt");
  if (!el) return;
  el.innerHTML = "ENEMY INCOMING"
    + "<span class=\"engage-sub\">Press <span class=\"engage-key\">L</span> "
    + "(or click <b>AUTO-LOCK+NAV</b> in the top bar) to automatically "
    + "engage the enemy and begin fighting.</span>";
  el.classList.remove("hidden");
  if (el._hideTimeout) clearTimeout(el._hideTimeout);
  el._hideTimeout = setTimeout(() => el.classList.add("hidden"), 6000);
}
function dismissEngagePrompt() {
  const el = document.getElementById("engage-prompt");
  if (!el) return;
  if (el._hideTimeout) clearTimeout(el._hideTimeout);
