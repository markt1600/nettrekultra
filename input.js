...
// Manual phaser fire — used by both the P key and the on-screen PHASER button.
// Locks on if a target is in cone+range; else discharges in current heading.
function firePhaserManual(me, world) {
  let ok = false;
  if (me.targetLock) {
    const t = world.ships.find(o => o.id === me.targetLock && o.alive);
...
// Manual torpedo fire — used by both the T key and the on-screen TORP button.
function fireTorpManual(me, world) {
...
function attachInput(world) {
  ...
  // ---- Weapons panel + speed meter (left of tactical canvas) ----
  const btnSh = document.getElementById("btn-shields");
  ...
  const btnFl = document.getElementById("btn-fire-flare");
  if (btnFl) btnFl.addEventListener("click", () => {
    const me = world.playerShip;
    if (!me || !me.alive) return;
    const r = deployFlare(me, world);
    pushMessage(world, r.msg, r.msgKind);
  });
  const btnPh = document.getElementById("btn-fire-phaser");
  if (btnPh) btnPh.addEventListener("click", () => {
    const me = world.playerShip;
    if (me && me.alive) firePhaserManual(me, world);
  });
  const btnTp = document.getElementById("btn-fire-torp");
  if (btnTp) btnTp.addEventListener("click", () => {
    const me = world.playerShip;
    if (me && me.alive) fireTorpManual(me, world);
  });
  const speedMeter = document.getElementById("speed-meter");
  if (speedMeter) speedMeter.addEventListener("click", (e) => {
    const me = world.playerShip;
    if (!me || !me.alive) return;
    // Each warp level (0–9) is its own clickable row. Read the integer right
    // off data-warp so the click always snaps to a clean warp value rather
    // than depending on pixel math.
    const lvl = e.target.closest(".speed-level");
    if (!lvl) return;
    const v = parseInt(lvl.getAttribute("data-warp"), 10);
  ...
  document.addEventListener("keydown", e => {
    ...
    if (k === "l") {
      ...
      // Player took the cue — dismiss the "ENEMY INCOMING" prompt if it's up.
      if (world.autoLockAndNavigate && typeof dismissEngagePrompt === "function") dismissEngagePrompt();
    ...
    if (k === "p") { firePhaserManual(me, world); return; }
    if (k === "t") { fireTorpManual(me, world); return; }
